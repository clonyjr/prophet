...
def prophet_fit(df, prophet_model, today_index, lookback_days=None, predict_days=21, is_df_hourly=False,init_hourly='10:00', finish_hourly='16:00'):
    """
    ...
    if not lookback_days:
        print('Treinando para o período de {} a {} ({} dias)'.format(df['ds'][0],
                                                            df['ds'][today_index - 1],
                                                            today_index))
    else:
        baseline_ts = df['ds'][today_index - lookback_days:today_index]
        baseline_y = df.y[today_index - lookback_days:today_index]
        print('Treinando para o período de {} a {} ({} dias)'.format(df['ds'][today_index - lookback_days],
                                                            df['ds'][today_index - 1],
                                                            lookback_days))
    print('Previsão de {} até {} ({} dias)'.format(df['ds'][today_index],
    ...
    if is_df_hourly:
        ...
        future = future.between_time('10:00', '16:00')
...
def prophet_plot(df, fig, today_index, lookback_days=None, predict_days=21, outliers=list()):
    ...
    ax.text(x_pydatetime[int((start + today_index) / 2)], ymin + (ymax - ymin) / 20, 'Area de baseline')
    ax.text(x_pydatetime[int((today_index * 2 + predict_days) / 2)], ymin + (ymax - ymin) / 20, 'Area de previsão')

    # re-organize the legend
    patch1 = mpatches.Patch(color='red', label='Outlier')
    patch2 = mpatches.Patch(color='orange', label='Real')
    patch3 = mpatches.Patch(color='skyblue', label='Previsão e intervalo')
...
def get_outliers(df, forecast, today_index, predict_days=21):
    ...
    for i in range(df_pred.shape[0]):
        ...
        if actual_value < df_pred['lower_y'][i] or actual_value > df_pred['upper_y'][i]:
            ...
            print('=====')
            print('valor real {} ficou fora do intervalo de previsão'.format(actual_value))
            print('intervalo: de {} até {}'.format(df_pred['lower_y'][i], df_pred['upper_y'][i]))
            print('Data: {}'.format(str(df_pred.index[i])[:10]))
